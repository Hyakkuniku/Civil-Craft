...
public class Bar : MonoBehaviour
{
    ...
    private void RemoveConnections()
    {
        ...
        Point previousEnd = endPoint;

        if (previousStart != null)
            while (previousStart.ConnectedBars.Remove(this)) { }
        if (previousEnd != null)
            while (previousEnd.ConnectedBars.Remove(this)) { }

        // A shared cap must stay anchored if another pier still supports it, but
        // must immediately return to a normal node after its final pier is removed.
        if (previousStart != null) previousStart.RefreshAnchorState();
        if (previousEnd != null && previousEnd != previousStart) previousEnd.RefreshAnchorState();
    }
    ...
    public void NormalizeEndpointOrder()
    {
        ...
        // Bars are commonly initialized before their endpoints are assigned, so
        // OnEnable cannot always register these connections for us.
        if (!startPoint.ConnectedBars.Contains(this)) startPoint.ConnectedBars.Add(this);
        if (!endPoint.ConnectedBars.Contains(this)) endPoint.ConnectedBars.Add(this);

        if (materialData.isPier)
        {
            ClearLegacyPierAnchorPromotion(startPoint);
            ClearLegacyPierAnchorPromotion(endPoint);

            // Pier support is derived from this active connection. Do not promote
            // either endpoint to an authored scene anchor: deletion must be able
            // to return a shared upper node to its normal movable state.
            RefreshPierEndpointAnchors();
        }
    ...
    private void RefreshPierEndpointAnchors()
    {
        if (materialData == null || !materialData.isPier) return;
        if (startPoint != null) startPoint.RefreshAnchorState();
        if (endPoint != null && endPoint != startPoint) endPoint.RefreshAnchorState();
    }

    private static void ClearLegacyPierAnchorPromotion(Point point)
    {
        // Runtime points were generated by the player. Older builds permanently
        // promoted both pier endpoints; clear that stale save/runtime state while
        // preserving authored scene anchors (Runtime == false).
        if (point != null && point.Runtime)
            point.originalIsAnchor = false;
    }

    /// <summary>
    /// The pier cap is a visual connection plate, not a driving or structural
    /// collision surface. The anchored upper node provides the connection.
    /// </summary>
    public void RemovePierCapColliders()
    {
        Transform cap = pierCapInstance != null ? pierCapInstance.transform : transform.Find("PierCap");
        if (cap == null) return;

        foreach (Collider capCollider in cap.GetComponentsInChildren<Collider>(true))
        {
            if (capCollider == null) continue;
            capCollider.enabled = false;
            if (Application.isPlaying) Destroy(capCollider);
            else DestroyImmediate(capCollider);
        }
    }
